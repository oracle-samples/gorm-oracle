...
func TestUpdate(t *testing.T) {
	...
	if err := DB.Where("\"id\" = ?", user.ID).First(&result).Error; err != nil {
		...
		CheckUserSkipUpdatedAt(t, result, *user)
	}
	...
	if err := DB.Where("\"id\" = ?", user.ID).First(&result2).Error; err != nil {
		...
		CheckUserSkipUpdatedAt(t, result2, *user)
	}
	...
	if err := DB.Where("\"id\" = ?", user.ID).First(&result3).Error; err != nil {
		...
		CheckUserSkipUpdatedAt(t, result3, *user)
	}
	...
	if err := DB.Where("\"id\" = ?", user.ID).First(&result4).Error; err != nil {
		...
		CheckUserSkipUpdatedAt(t, result4, *user)
	}
...
func TestUpdates(t *testing.T) {
	...
	CheckUserSkipUpdatedAt(t, user1, *users[0])
	CheckUserSkipUpdatedAt(t, user2, *users[1])
	...
	user3.Age += 100
	tests.AssertObjEqual(t, user4, user3, "UpdatedAt", "Age")
}
...
func TestUpdateColumn(t *testing.T) {
	...
	tests.AssertEqual(t, lastUpdatedAt.UnixNano(), users[1].UpdatedAt.UnixNano())
	...
	users[1].Age += 50
	CheckUserSkipUpdatedAt(t, user3, *users[1])
	...
	CheckUserSkipUpdatedAt(t, user5, *users[0])
	CheckUserSkipUpdatedAt(t, user6, *users[1])
}
...
func TestSelectWithUpdate(t *testing.T) {
	...
	tests.AssertObjEqual(t, result2, result, "Name", "Account", "Toys", "Manager", "ManagerID", "Languages")
	...
	// tests.AssertObjEqual(t, result, user, "UpdatedAt")

	var result3 User
	DB.First(&result3, result.ID)
	tests.AssertObjEqual(t, result, result3, "Name", "Age")
	...
	tests.AssertObjEqual(t, result, User{Name: "update_with_select"}, "Name", "Age")
}
...
func TestOmitWithUpdate(t *testing.T) {
	...
	tests.AssertObjEqual(t, result2, result, "Age", "Pets", "Company", "CompanyID", "Team", "Friends")
}
...
func TestUpdateColumnsSkipsAssociations(t *testing.T) {
	...
	newAge := uint(100)
	user.Account.AccountNumber = "new_account_number"
	...
	if result.Account.AccountNumber != user.Account.AccountNumber {
		t.Errorf("account number should not been changed, expects: %v, got %v", user.Account.AccountNumber, result.Account.AccountNumber)
	}
}
...
func TestUpdateWithDiffSchema(t *testing.T) {
	...
	tests.AssertEqual(t, "update-diff-schema-2", user.Name)
}
...
func TestUpdateFrom(t *testing.T) {
	...
	if rowsAffected := DB.Model(&User{}).Clauses(clause.From{Tables: []clause.Table{{Name: "accounts"}}}).Where("accounts.user_id = users.id AND accounts.number = ? AND accounts.deleted_at IS NULL", users[0].Account.AccountNumber).Update("name", "franco").RowsAffected; rowsAffected != 1 {
		t.Errorf("should only update one record, but got %v", rowsAffected)
	}
	...
	if rowsAffected := DB.Model(&User{}).Clauses(clause.From{Tables: []clause.Table{{Name: "accounts"}}}).Where("accounts.user_id = users.id AND accounts.number IN ? AND accounts.deleted_at IS NULL", []string{users[0].Account.AccountNumber, users[1].Account.AccountNumber}).Update("name", gorm.Expr("accounts.number")).RowsAffected; rowsAffected != 2 {
	...
	for _, user := range results {
		if user.Name != user.Account.AccountNumber {
			t.Errorf("user's name should be equal to the account's number %v, but got %v", user.Account.AccountNumber, user.Name)
		}
	}
}
func TestUpdateWithMultipleWhere(t *testing.T) {
	user := *GetUser("multi_where", Config{})
	user.Age = 30
	DB.Create(&user)

	err := DB.Model(&User{}).Where("\"name\" = ? AND \"age\" = ?", "multi_where", 30).Update("age", 40).Error
	if err != nil {
		t.Errorf("update with multiple WHERE should succeed, got: %v", err)
	}

	var updated User
	DB.First(&updated, user.ID)
	if updated.Age != 40 {
		t.Errorf("expected age to be updated to 40, got: %v", updated.Age)
	}
}

func TestUpdatePrimaryKeyField(t *testing.T) {
	user := *GetUser("update_pk", Config{})
	DB.Create(&user)

	if err := DB.Model(&user).Updates(map[string]interface{}{"id": 9999, "name": "new name"}).Error; err != nil {
		t.Errorf("update should not fail even if ID is present, but got: %v", err)
	}

	var updated User
	if err := DB.First(&updated, 9999).Error; err != nil {
		t.Fatalf("expected record with updated ID, got error: %v", err)
	}
	if updated.Name != "new name" {
		t.Errorf("name should be updated, got: %v", updated.Name)
	}
}
func TestBatchUpdateSlice(t *testing.T) {
	users := []*User{
		GetUser("batch1", Config{}),
		GetUser("batch2", Config{}),
		GetUser("batch3", Config{}),
		GetUser("batch4", Config{}),
		GetUser("batch5", Config{}),
	}
	DB.Create(&users)
	...
	for _, u := range users {
		...
		if updated.Age != 99 {
			t.Errorf("user age should be 99 after batch update, got %v", updated.Age)
		}
	}
}
func TestMixedSaveBatch(t *testing.T) {
	...
	newUser := &User{Name: "new_user", Age: 33}
	...
	if err := DB.Save(&users).Error; err != nil {
		t.Errorf("batch save (mixed insert/update) failed: %v", err)
	}
	...
	for _, u := range users {
		...
		DB.First(&out, u.ID)
		if out.Age != 99 {
			t.Errorf("user %v age not updated to 99", u.Name)
		}
	}
}

func TestUpdatesWithZeroValue(t *testing.T) {
	user := *GetUser("zero_value", Config{})
	user.Age = 25
	user.Active = true
	DB.Create(&user)

	if err := DB.Model(&user).Updates(User{Age: 0, Active: false}).Error; err != nil {
		t.Errorf("failed to update with zero values: %v", err)
	}

	var updated1 User
	DB.First(&updated1, user.ID)
	if updated1.Age == 0 || updated1.Active == false {
		t.Errorf("zero values should not be updated by default, got age: %v, active: %v", updated1.Age, updated1.Active)
	}

	if err := DB.Model(&user).Select("Age", "Active").Updates(User{Age: 0, Active: false}).Error; err != nil {
		t.Errorf("failed to update with zero values using Select: %v", err)
	}

	var updated2 User
	DB.First(&updated2, user.ID)
	if updated2.Age != 0 || updated2.Active != false {
		t.Errorf("zero values should be updated with explicitly select, got age: %v, active: %v", updated2.Age, updated2.Active)
	}
}

func TestUpdateStringFieldToNull(t *testing.T) {
	user := *GetUser("update_to_null", Config{})
	DB.Create(&user)

	// Update Name to null using map
	if err := DB.Model(&user).Update("name", nil).Error; err != nil {
		t.Errorf("failed to update field to null: %v", err)
	}
	...
	if result.Name != "" {
		t.Errorf("expected name to be null/empty string, but got: '%v'", result.Name)
	}
}
func TestUpdateColumnsWithNulls(t *testing.T) {
	user := *GetUser("update_columns_null", Config{})
	user.Name = "non-null"
	DB.Create(&user)

	err := DB.Model(&user).UpdateColumns(map[string]interface{}{"name": nil}).Error
	if err != nil {
		t.Errorf("UpdateColumns should allow nil, got: %v", err)
	}

	var result User
	DB.First(&result, user.ID)
	if result.Name != "" {
		t.Errorf("Expected name to be empty, got: %v", result.Name)
	}
}
func TestUpdatesWithStructPointer(t *testing.T) {
	user := *GetUser("struct_pointer", Config{})
	DB.Create(&user)

	update := &User{Age: 77}
	if err := DB.Model(&user).Updates(update).Error; err != nil {
		t.Errorf("failed to update with struct pointer: %v", err)
	}

	var updated User
	DB.First(&updated, user.ID)
	if updated.Age != 77 {
		t.Errorf("expected age 77, got: %v", updated.Age)
	}
}
...
// Struct using custom data type
type CustomUser struct {
	ID       uint
	Name     string
	Location Location `gorm:"type:VARCHAR(64)"`
}

func TestUpdateCustomDataType(t *testing.T) {
	DB.Migrator().DropTable(&CustomUser{})
	DB.AutoMigrate(&CustomUser{})

	user := CustomUser{Name: "geo-user", Location: Location{X: 1, Y: 2}}
	DB.Create(&user)

	if err := DB.Model(&user).Update("Location", Location{X: 100, Y: 200}).Error; err != nil {
