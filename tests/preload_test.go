...
func TestPreloadWithConds(t *testing.T) {
	...
	for idx, user := range users2[1:2] {
		if user.Account.AccountNumber != "" {
			t.Errorf("No account should found for user %v but got %v", idx+2, user.Account.AccountNumber)
		}
	}
...
func TestNestedPreloadWithNestedJoin(t *testing.T) {
	...
	tests.AssertEqual(t, find1, value1)
	...
	tests.AssertEqual(t, find2, value2)
	...
	tests.AssertEqual(t, len(finds), 2)
	tests.AssertEqual(t, finds[0], value1)
	tests.AssertEqual(t, finds[1], value2)
}
...
func TestMergeNestedPreloadWithNestedJoin(t *testing.T) {
	...
	tests.AssertEqual(t, result, users)
	tests.AssertEqual(t, len(query), 2) // Check preload queries are merged
...
func TestNestedPreloadWithPointerJoin(t *testing.T) {
	...
	tests.AssertEqual(t, find1, value)
}
...
func TestEmbedPreload(t *testing.T) {
	...
	for _, test := range testList {
		t.Run(test.name, func(t *testing.T) {
			...
			tests.AssertEqual(t, actual, test.expect)
		})
	}
}
